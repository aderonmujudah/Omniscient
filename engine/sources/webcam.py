...
# Capture resolution, chosen by the project owner on 2026-08-27 over the 640x480 this previously
# requested. Measured on the reference device, this mode places 133.8 px between the irises
# against 46.9 px at 640x480, and sustains 28.7 to 29.9 samples a second across recordings up to
# 240 s, so the calibration collection window still holds about twenty samples. The gaze feature
# is an iris displacement normalised by eye width, so its value does not vary with resolution but
# its precision does: landmark jitter is roughly constant in pixels, and at 640x480 the jitter
# within a single held fixation was the size of the whole signal range across the screen.
#
# The device delivers a different aspect ratio at some of its modes, so it crops rather than
# scales between them. A focal length in pixels therefore holds only for the mode it was measured
# at and does not convert to another; changing these constants invalidates it.
CAPTURE_WIDTH = 1920
CAPTURE_HEIGHT = 1080
...
class WebcamSource(GazeSource):
    ...
    def start(self) -> None:
        ...
        if actual_w != CAPTURE_WIDTH or actual_h != CAPTURE_HEIGHT:
            logger.warning(
                "Camera did not honor %dx%d request. Actual resolution: %dx%d. Every geometry "
                "derived from the focal length assumes the requested width.",
                CAPTURE_WIDTH, CAPTURE_HEIGHT, actual_w, actual_h)
